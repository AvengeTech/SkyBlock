...
class Clipboard extends Entity implements ChunkLoader{
	...
	public function __construct(Location $loc, ?CompoundTag $nbt = null, float $scale = 1.0){
		...
		$this->setNametag(TextFormat::RED . TextFormat::BOLD . "STATS");
		$this->setNametagAlwaysVisible(true);
		$this->setScale($scale);
	}

	public static function getNetworkTypeId() : string{
		return "game:clipboard";
	}
	...
	public function attack(EntityDamageEvent $source) : void{
		...
		if($source instanceof EntityDamageByEntityEvent){
			...
			if($player instanceof Player) {
				...
				$player->showModal(new StatsUi($player->getGameSession()));
			}
		}
	}

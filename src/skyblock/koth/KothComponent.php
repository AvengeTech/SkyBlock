...
class KothComponent extends SaveableComponent{
	...
	public function getName() : string{ return "koth"; }
	...
	public function getStats(int $type = KothStat::TYPE_ALLTIME) : ?KothStat{
		return isset($this->stats[$type]) ? $this->stats[$type] : null;
	}

	public function getKills(int $type = KothStat::TYPE_ALLTIME) : int{
	...
	public function getDeaths(int $type = KothStat::TYPE_ALLTIME) : int{
	...
	public function getWins(int $type = KothStat::TYPE_ALLTIME) : int{
	...
	public function resetStats(int $type = KothStat::TYPE_WEEKLY) : void{
	...
	public function finishLoadAsync(?ComponentRequest $request = null) : void{
		...
		for($i = 0; $i <= 2; $i++){
			if(!isset($this->stats[$i])){
				$this->stats[$i] = new KothStat($this, $i);
			}
		}

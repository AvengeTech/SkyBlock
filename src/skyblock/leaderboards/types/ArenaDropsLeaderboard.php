...
class ArenaDropsLeaderboard extends Leaderboard implements MysqlUpdate{
	...
	public function calculate() : void{
		SkyBlock::getInstance()->getSessionManager()->sendStrayRequest(new MySqlRequest("update_leaderboard_" . $this->getType() . "_" . $this->typeToName(), new MySqlQuery(
			"main",
			"SELECT xuid, supply_drops FROM combat_stats WHERE ttype=" . $this->getStatType() . " ORDER BY supply_drops DESC LIMIT " . $this->getSize() . ";",
			[]
			...
			Core::getInstance()->getUserPool()->useUsers($xuids, function(array $users) use($rows) : void{
				$texts = [TextFormat::RED . TextFormat::BOLD . TextFormat::EMOJI_STAR . " Supply Drops (" . $this->typeToName() . ") " . TextFormat::EMOJI_STAR];
				$i = 1;
				foreach($rows as $row){
					$texts[($gt = $users[$row["xuid"]]->getGamertag())] =
						TextFormat::RED . $i . ". " .
						TextFormat::YELLOW . $gt . " " . TextFormat::GRAY . "- " .
						TextFormat::AQUA . number_format($row["supply_drops"] ?? 0);
					$i++;
				}
				$this->texts = $texts;

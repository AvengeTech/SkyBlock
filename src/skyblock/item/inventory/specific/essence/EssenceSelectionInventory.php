...
namespace skyblock\item\inventory\specific\essence;
...
use skyblock\item\ui\specific\essence\EditAscensionUI;
use skyblock\item\ui\specific\essence\EditKnowledgeUI;
use skyblock\item\ui\specific\essence\EditSuccessUI;
use skyblock\SkyBlock;
use skyblock\SkyBlockPlayer;

class EssenceSelectionInventory extends SimpleInventory implements TempInventory{
	...
	public function setup() : self{
		...
		$this->addItem(
			ItemRegistry::ESSENCE_OF_ASCENSION()->setup(ED::RARITY_DIVINE)->init()->setLore([
				"big forehead shane"
			])->setCount($this->increaseCount),
			ItemRegistry::ESSENCE_OF_KNOWLEDGE()->setup(ED::RARITY_DIVINE)->init()->setLore([
				"shane big forehead"
			])->setCount($this->increaseCount),
			ItemRegistry::ESSENCE_OF_SUCCESS()->setup(ED::RARITY_DIVINE)->init()->setLore([
				"forehead shane big"
			])->setCount($this->increaseCount),
		);
	...
	public function handle(Player $player, Item $item) : bool{
		...
		if($item instanceof Banner){
			...
			SkyBlock::getInstance()->getScheduler()->scheduleDelayedTask(new ClosureTask(function() use($player, $item){
				$ui = match(true){
					$item instanceof EssenceOfAscension => new EditAscensionUI($item),
					$item instanceof EssenceOfKnowledge => new EditKnowledgeUI($item),
					$item instanceof EssenceOfSuccess => new EditSuccessUI($item),
					default => null,
				};

				if(is_null($ui)){
					$player->sendMessage(TF::RI . "Could not open correct Edit UI for the essence you selected.");
					return;
				}
	...
	public function getName() : string{ return "EssenceSelectionInventory"; }

	public function getDefaultSize() : int{ return 54; }

	public function getTitle() : string{
		return TF::BOLD . TF::AQUA . "Essence" . TF::GOLD . " Selection";
	}
